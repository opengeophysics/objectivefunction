...
class BaseObjectiveFunction(properties.HasProperties):
    ...
    def __init__(self, nP=None, **kwargs):
        if nP is not None:
            self._nP = nP
        super(BaseObjectiveFunction, self).__init__(**kwargs)
    ...
    def __add__(self, objfct2):
        ...
        if self.__class__.__name__ != 'ComboObjectiveFunction':
            self = 1 * self

        # not isinstance(objfct2, ComboObjectiveFunction):
        if objfct2.__class__.__name__ != 'ComboObjectiveFunction':
            objfct2 = 1 * objfct2
...
class L2ObjectiveFunction(BaseObjectiveFunction):
    ...
    .. math::
        ...
        \phi = \\frac{1}{2}||\mathbf{W} \mathbf{m}||^2_2
    """
